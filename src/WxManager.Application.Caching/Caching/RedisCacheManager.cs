...
namespace WxManager.Application.Caching
{
    ...
    public partial class RedisCacheManager : IStaticCacheManager
    {
        ...
        protected virtual async Task RemoveAsync(string key)
        {
            ...
            if (key.Equals(WxManagerCachingDefaults.RedisDataProtectionKey, StringComparison.OrdinalIgnoreCase))
                return;
        ...
        protected virtual async Task RemoveByPatternAsync(string pattern)
        {
            ...
            foreach (var endPoint in _connectionWrapper.GetEndPoints())
            {
                ...
                //we should always persist the data protection key list
                keys = keys.Where(key => !key.ToString().Equals(WxManagerCachingDefaults.RedisDataProtectionKey, StringComparison.OrdinalIgnoreCase));
        ...
        public virtual T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            if ((cacheTime ?? WxManagerCachingDefaults.CacheTime) > 0)
                this.SetAsync(key, result, cacheTime ?? WxManagerCachingDefaults.CacheTime).Wait();
